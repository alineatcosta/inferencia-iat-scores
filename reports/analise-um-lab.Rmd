...
iat %>% 
    group_by(sex) %>% 
    summarise(media = mean(d_art), sd=sd(d_art), n = n())
```
...
Em média, as mulheres que participaram do experimento tiveram uma associação implícita (medida pelo IAT) com a matemática **negativa** e **média** (média 0.57, desv. padrão 0.42, N = 65). Homens tiveram uma associação também **negativa** com a matemática, mas **menor** que a das mulheres (média 0.40, desv. padrão 0.51, N = 48). Houve portanto uma **pequena** diferença entre homens e mulheres (diferença das médias -0.17, 95% CI [-0.34, 0.01]). A partir desta amostra, estimamos que mulheres parecem ter uma associação negativa mais forte, porém a diferença parece ser pequena [-0.34, 0.01]. O intervalo inclui o 0, portanto pode não haver diferença entre homens e mulheres. 
